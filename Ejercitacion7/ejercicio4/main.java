...
public class main {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < jugadores.length; i++) {
            s.agregarJugador(jugadores[i]);
        }

        s.mostrarConvocados();

        s.mostrarNoConvocados();

        s.contarPorSeleccion();

        s.jugadorMasJoven();

        s.jugadorMasVeterano();

        s.buscarPorPosicion("Delantero");

        s.buscarPorEquipo("Real Madrid");

        s.contarPorPosicion();

        s.estaConvocado("Paulo Dybala");

        s.promedioEdad();

        s.ordenarPorEdad();

        s.eliminarJugador("Antoine Griezmann");
    }
}